...
export function PaymentDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `${isPayment ? "Payment" : "Expense"} recorded successfully`,
        type: "success",
      });
      ...
      Alert.alert("Error", `Failed to record ${formData.type}. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <View className="bg-blue-600 px-4 py-3 pt-12 flex-row items-center justify-between">
          <View className="flex-1">
            <Text className="text-white text-lg font-semibold">
              {formData.type === "payment" ? "Record Payment" : "Record Expense"}
            </Text>
        ...
        <ScrollView className="flex-1 px-4 py-4">
          ...
          <View className="mb-6">
            <View className={`rounded-lg p-4 border ${
              ...
              <Text className="text-sm font-medium text-gray-700 mb-1">Current Balance</Text>
              <Text className={`text-xl font-bold ${
                ...
                {balanceInfo.isPositiveBalance ? "Owed: " : balanceInfo.totalBalance < 0 ? "Credit: " : "Paid: "}
                {getCurrencySymbol(reservation?.currency || "LKR")}{balanceInfo.displayBalance.toFixed(2)}
              </Text>
              ...
              <View className="mt-2 pt-2 border-t border-gray-200">
                <View className="flex-row justify-between">
                  ...
                  <Text className="text-sm font-medium">
                    {getCurrencySymbol(reservation?.currency || "LKR")}{reservation?.total_amount || 0}
                  </Text>
                </View>
                <View className="flex-row justify-between">
                  <Text className="text-sm text-gray-600">Paid Amount:</Text>
                  <Text className="text-sm font-medium">
                    {getCurrencySymbol(reservation?.currency || "LKR")}{reservation?.paid_amount || 0}
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View className="mb-6">
            ...
            <View className="flex-row bg-gray-100 rounded-lg p-1">
              <TouchableOpacity
                onPress={() => setFormData(prev => ({...prev, type: "payment"}))}
          ...
          <View className="mb-6">
            ...
            <View className="flex-row space-x-2 mb-4">
              <TouchableOpacity
                onPress={() => setFormData(prev => ({...prev, amount: balanceInfo.displayBalance}))}
              ...
              <TouchableOpacity
                onPress={() => setFormData(prev => ({...prev, amount: balanceInfo.displayBalance / 2}))}
            ...
            <View className="border border-gray-300 rounded-lg">
              ...
              >
                ...
                {accounts?.map((account) => (
                  <Picker.Item
                    key={account.id}
                    label={`${account.name} (${getCurrencySymbol(account.currency)}${account.balance || 0})`}
                    value={account.id}
                  />
                ))}
              </Picker>
          ...
          <View className="bg-gray-50 rounded-lg p-4 mb-6">
            <Text className="text-sm font-medium text-gray-700 mb-2">Transaction Summary</Text>
            <View className="flex-row justify-between mb-1">
              <Text className="text-sm text-gray-600">Type:</Text>
              <Text className="text-sm font-medium capitalize">{formData.type}</Text>
            </View>
            <View className="flex-row justify-between mb-1">
